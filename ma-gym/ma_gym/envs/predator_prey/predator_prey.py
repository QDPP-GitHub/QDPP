...
class PredatorPrey(gym.Env):
    ...
    def __init__(self, grid_shape=(5, 5), n_agents=2, n_preys=1, prey_move_probs=(0.175, 0.175, 0.175, 0.175, 0.3),
                 full_observable=False, penalty=-0.5, step_cost=-0.0, prey_capture_reward=5, max_steps=100, pos_obs=False, prey_avoid_policy=False):
        ...
        self.prey_avoid_policy = prey_avoid_policy
        ...
        self.state_num = grid_shape[0] * grid_shape[1] * 4
        self.state_idx_bias = [0] * self.n_agents

        # agent pos (2), prey (25), step (1)
        
        if self.pos_obs:
            self.mask_pos_size = 2 * (self.n_agents - 1 + self.n_preys)
            self._obs_high = np.array([1., 1.] * (self.n_agents - 1 + self.n_preys) + [1.0])
            self._obs_low = np.array([0., 0.] * (self.n_agents - 1 + self.n_preys)+ [0.0])
        else:
            self.mask_size = mask_size = np.prod(self._agent_view_mask)
    ...
    def __init_full_obs(self):
        ...
        for prey_i in range(self.n_preys):
            while True:
                pos = [random.randint(0, self._grid_shape[0] - 1), random.randint(0, self._grid_shape[1] - 1)]
                #  and (self._neighbour_agents(pos)[0] == 0)
                if self._is_cell_vacant(pos):
    ...
    def get_agent_obs(self):
        ...
        for agent_i in range(self.n_agents):
            ...
            if self.pos_obs:
                prey_count = 0
                agent_count = 0
                _prey_pos = np.ones(2 * self.n_preys)
                _agent_pos = np.ones(2 * (self.n_agents - 1))
            else:
            ...
            for row in range(max(0, pos[0] - 2), min(pos[0] + 2 + 1, self._grid_shape[0])):
                for col in range(max(0, pos[1] - 2), min(pos[1] + 2 + 1, self._grid_shape[1])):
                    if PRE_IDS['agent'] in self._full_obs[row][col]:
                        if self.pos_obs and pos[0]!=row and pos[1]!=col:
                            _agent_pos[agent_count * 2] = 1. * row / self._grid_shape[0]
                            _agent_pos[agent_count * 2 + 1] = 1. * col / (self._grid_shape[1] - 1)
                            agent_count += 1

                    if PRE_IDS['prey'] in self._full_obs[row][col]:
                        if self.pos_obs:
                            _prey_pos[prey_count * 2] = 1. * row / self._grid_shape[0]
                            _prey_pos[prey_count * 2 + 1] = 1. * col / (self._grid_shape[1] - 1)
                            prey_count += 1
                        else:
            ...
            if self.pos_obs:
                _agent_i_obs += _agent_pos.flatten().tolist() 
                if agent_count == 0 and prey_count == 0:
                    self.state_idx_bias[agent_i] == 0
                elif agent_count == 0 and prey_count > 0:
                    self.state_idx_bias[agent_i] == 1
                elif agent_count > 0 and prey_count == 0:
                    self.state_idx_bias[agent_i] == 2
                elif agent_count > 0 and prey_count > 0:
                    self.state_idx_bias[agent_i] == 3
    ...
    def get_state(self):
        ...
        for i in range(self.n_agents):
            ...
            idx = idx * 4 + self.state_idx_bias[i]
            pos_idxs.append(idx)
    ...
    def step(self, agents_action):
        ...
        return self.get_agent_obs(), rewards, self._agent_dones, {'prey_alive': sum(self._prey_alive)}
