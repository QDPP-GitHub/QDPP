...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # Discriminator
        mac_out_2 = mac_out.clone()
        mac_out_2[avail_actions == 0] = -9999999
        q_softmax_actions = th.nn.functional.softmax(mac_out_2[:, :-1], dim=3)

        if self.args.hard_qs:
            _, maxs = th.max(mac_out_2[:, :-1], dim=3, keepdim=True)
