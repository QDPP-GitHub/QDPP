...
class QLearner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        if self.args.mixer == "qtran_base":
            ...
            nopt_values = chosen_action_ind_qvals.sum(dim=2).reshape(-1, 1) - joint_qs.detach() + vs # Don't use target networks here either
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            if self.args.mixer == "qtran_base":
                ...
                self.logger.log_stat("agent_indiv_qs", ((chosen_action_ind_qvals * mask).sum().item()/(mask_elems * self.args.n_agents)), t_env)
                if self.args.device != 'cuda' and self.args.mixer is not None:
                    # print(joint_qs.shape, chosen_action_ind_qvals.shape)
                    jointq_ind_q = joint_qs - chosen_action_ind_qvals.sum(dim=-1).unsqueeze(2).reshape(joint_qs.shape)
